...
1. **Bloqueio de Gerência (WebGUI):** Impede que usuários comuns tentem acessar o painel administrativo do firewall.
2. **Prevenção contra Ataques Cibernéticos:** Fechamento de portas críticas (como a 445/SMB) para evitar propagação de malwares.
...
Tentativa de acesso ao IP do firewall (`192.168.10.1`) a partir do navegador do Ubuntu.
![WebGUI Bloqueada](webgui_bloqueada.PNG)
*O navegador exibe "Timed Out", confirmando que a regra de bloqueio está descartando os pacotes silenciosamente.*
...
*Os logs em vermelho exibem claramente a ação de **Block**, a interface (CLIENTES), o IP de origem (`192.168.10.100`) e a **Label** da regra que causou o bloqueio.*
