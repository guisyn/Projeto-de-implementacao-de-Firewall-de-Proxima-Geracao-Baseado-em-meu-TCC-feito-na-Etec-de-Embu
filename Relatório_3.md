...
O acesso administrativo via SSH foi habilitado para permitir a gerência do sistema através de linha de comando (CLI). O teste de sucesso foi realizado a partir da máquina física (Windows) conectada à rede de **Gerenciamento**.

![Acesso SSH Windows](ssh_windows.PNG)
*Conexão SSH estabelecida com sucesso via terminal Windows, exibindo o menu de administração do OPNsense 25.7.*
...
Tentativa de conexão a partir do host Ubuntu (`192.168.10.100`) para o gateway (`192.168.10.1`).
...
*Logs exibindo o bloqueio da porta 22 (tcp) na interface CLIENTES, com a label "Bloqueio de acesso remoto para rede Clientes".*
