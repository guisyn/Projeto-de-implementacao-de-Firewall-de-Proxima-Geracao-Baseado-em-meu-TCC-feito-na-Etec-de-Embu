...
Este relatório descreve a implementação de uma VPN (Virtual Private Network) para acesso remoto seguro, detalhando a configuração de segurança e os testes de validação em ambiente de laboratório.
...
Cabe ressaltar que os testes foram conduzidos sob limitações de **Double NAT**. Para garantir a funcionalidade no laboratório, foi utilizado o IP da interface de gerenciamento/WAN como Hostname de exportação, contornando restrições de roteamento do hardware físico.
